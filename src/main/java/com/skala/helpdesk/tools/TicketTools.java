...
public class TicketTools {
    ...
    public TicketTools(OrderRepository orderRepository, TicketRepository ticketRepository, MeterRegistry registry) {
    ...
    public String requestRefund(
            @ToolParam(description = "주문번호. 대화에서 이미 조회했던 주문번호가 있으면 그걸 그대로 쓴다. 예: 12345") String orderId,
            @ToolParam(description = "환불 사유. 사용자가 말한 표현을 그대로 옮긴다. 예: '단순 변심', '사이즈가 안 맞음'. "
                    + "사용자가 사유를 전혀 언급하지 않았을 때만 되물어라.") String reason,
            ToolContext ctx) {
        ...
        try {
            ...
            log.info("[{}]   도구 requestRefund({}) {}ms",
                    MDC.get(HelpDeskService.TRACE_ID), orderId, (System.nanoTime() - start) / 1_000_000);
